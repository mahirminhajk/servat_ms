...
export const generateTokenProvider = (payload: { id: number, phone: string }): string => {
    return jwt.sign({
        ...payload,
        provider: true
    }, process.env.JWT_SECRET_PROVIDER!, { expiresIn: process.env.JWT_EXPIRES_IN_PROVIDER });
};

export const verifyTokenProvider = (token: string): { id: number, phone: string } | null => {
    try {
        const decode = jwt.verify(token, process.env.JWT_SECRET_PROVIDER!);
        if (typeof decode === "object" && decode.hasOwnProperty("id") && decode.hasOwnProperty("phone") && decode.hasOwnProperty("provider")) {
            return { id: decode.id, phone: decode.phone };
        }
        return null;
    } catch (error) {
